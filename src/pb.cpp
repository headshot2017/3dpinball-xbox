...
void pb::keyup()
{
	...
	if (xbox_input::MoveLeftPaddleUp())
	{
		MainTable->Message(1001, time_now);
	}

	if (xbox_input::MoveRightPaddleUp())
	{
		MainTable->Message(1003, time_now);
	}

	if (xbox_input::LaunchBallUp())
	{
		MainTable->Message(1005, time_now);
	}

	if (xbox_input::NudgeRightUp())
	{
		nudge::un_nudge_right(0, nullptr);
	}
	else if (xbox_input::NudgeLeftUp())
	{
		nudge::un_nudge_left(0, nullptr);
	}
	else if (xbox_input::NudgeUpUp())
	{
...
void pb::keydown()
{
	...
	if (xbox_input::MoveLeftPaddleDown())
	{
		MainTable->Message(1000, time_now);
	}

	if (xbox_input::MoveRightPaddleDown())
	{
		MainTable->Message(1002, time_now);
	}

	if (xbox_input::LaunchBallDown())
	{
		MainTable->Message(1004, time_now);
	}

	if (xbox_input::NudgeRightDown())
	{
		if (!MainTable->TiltLockFlag)
			nudge::nudge_right();
	}

	if (xbox_input::NudgeLeftDown())
	{
		if (!MainTable->TiltLockFlag)
			nudge::nudge_left();
	}

	if (xbox_input::NudgeUpDown())
	{
		if (!MainTable->TiltLockFlag)
			nudge::nudge_up();
	}
